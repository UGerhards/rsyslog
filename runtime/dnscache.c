...
static int key_equals_fn(void *key1, void *key2) {
    int RetVal = 0;
    ...
    switch (((struct sockaddr *)key1)->sa_family) {
        case AF_INET:
            RetVal = !memcmp(&((struct sockaddr_in *)key1)->sin_addr, &((struct sockaddr_in *)key2)->sin_addr,
 ...
 */
static void ATTR_NONNULL() entryDestruct(dnscache_entry_t *const etry) {
    if (etry->fqdn != NULL) prop.Destruct(&etry->fqdn);
    if (etry->fqdnLowerCase != NULL) prop.Destruct(&etry->fqdnLowerCase);
    if (etry->localName != NULL) prop.Destruct(&etry->localName);
    if (etry->ip != NULL) prop.Destruct(&etry->ip);
    free(etry);
}
 ...
 */
static rsRetVal ATTR_NONNULL() resolveAddr(struct sockaddr_storage *addr, dnscache_entry_t *etry) {
    DEFiRet;
    int error;
...
static rsRetVal ATTR_NONNULL() addEntry(struct sockaddr_storage *const addr, dnscache_entry_t **const pEtry) {
    int r;
...
static rsRetVal ATTR_NONNULL(1, 5) findEntry(struct sockaddr_storage *const addr,
                                             prop_t **const fqdn,
