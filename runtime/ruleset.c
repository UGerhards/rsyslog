...
DEFobjCurrIf(parser)
    ...
    static struct cnfparamdescr rspdescr[] = {{"name", eCmdHdlrString, CNFPARAM_REQUIRED},
 ...
 */
static rsRetVal iterateAllActions(rsconf_t *conf, rsRetVal (*pFunc)(void *, void *), void *pParam) {
...
static rsRetVal ATTR_NONNULL()
    execSet(const struct cnfstmt *const stmt, smsg_t *const pMsg, wti_t *const __restrict__ pWti) {
    struct svar result;
    DEFiRet;
...
static rsRetVal execUnset(struct cnfstmt *stmt, smsg_t *pMsg) {
    DEFiRet;
...
static rsRetVal invokeForeachBodyWith(struct cnfstmt *stmt, json_object *o, smsg_t *pMsg, wti_t *pWti) {
    struct svar v;
    v.datatype = 'J';
    v.d.json = o;
    DEFiRet;
...
static rsRetVal callForeachArray(struct cnfstmt *stmt, json_object *arr, smsg_t *pMsg, wti_t *pWti) {
    DEFiRet;
...
static rsRetVal callForeachObject(struct cnfstmt *stmt, json_object *arr, smsg_t *pMsg, wti_t *pWti) {
...
static rsRetVal ATTR_NONNULL() execForeach(struct cnfstmt *const stmt, smsg_t *const pMsg, wti_t *const pWti) {
...
static rsRetVal execPRIFILT(struct cnfstmt *stmt, smsg_t *pMsg, wti_t *pWti) {
    int bRet;
    DEFiRet;
    if ((stmt->d.s_prifilt.pmask[pMsg->iFacility] == TABLE_NOPRI) ||
        ((stmt->d.s_prifilt.pmask[pMsg->iFacility] & (1 << pMsg->iSeverity)) == 0))
        bRet = 0;
    else
        bRet = 1;
    ...
    if (bRet) {
        if (stmt->d.s_prifilt.t_then != NULL) CHKiRet(scriptExec(stmt->d.s_prifilt.t_then, pMsg, pWti));
    } else {
        if (stmt->d.s_prifilt.t_else != NULL) CHKiRet(scriptExec(stmt->d.s_prifilt.t_else, pMsg, pWti));
    }
finalize_it:
    RETiRet;
}
...
static int evalPROPFILT(struct cnfstmt *stmt, smsg_t *pMsg) {
    unsigned short pbMustBeFreed;
    uchar *pszPropVal;
    int bRet = 0;
    rs_size_t propLen;

    if (stmt->d.s_propfilt.prop.id == PROP_INVALID) goto done;

    pszPropVal = MsgGetProp(pMsg, NULL, &stmt->d.s_propfilt.prop, &propLen, &pbMustBeFreed, NULL);

    /* Now do the compares (short list currently ;)) */
    switch (stmt->d.s_propfilt.operation) {
        case FIOP_CONTAINS:
            if (rsCStrLocateInSzStr(stmt->d.s_propfilt.pCSCompValue, (uchar *)pszPropVal) != -1) bRet = 1;
            break;
        case FIOP_ISEMPTY:
            if (propLen == 0) bRet = 1; /* process message! */
            break;
        case FIOP_ISEQUAL:
            if (rsCStrSzStrCmp(stmt->d.s_propfilt.pCSCompValue, pszPropVal, propLen) == 0)
        ...
        case FIOP_ENDSWITH:
            ...
            break;
        case FIOP_REGEX:
            if (rsCStrSzStrMatchRegex(stmt->d.s_propfilt.pCSCompValue, (unsigned char *)pszPropVal, 0,
                                      &stmt->d.s_propfilt.regex_cache) == RS_RET_OK)
                bRet = 1;
            break;
        case FIOP_EREREGEX:
            if (rsCStrSzStrMatchRegex(stmt->d.s_propfilt.pCSCompValue, (unsigned char *)pszPropVal, 1,
                                      &stmt->d.s_propfilt.regex_cache) == RS_RET_OK)
                bRet = 1;
            break;
        case FIOP_NOP:
    ...
    if (stmt->d.s_propfilt.isNegated) bRet = (bRet == 1) ? 0 : 1;

    if (Debug) {
        if (stmt->d.s_propfilt.prop.id == PROP_CEE) {
            DBGPRINTF("Filter: check for CEE property '%s' (value '%s') ", stmt->d.s_propfilt.prop.name, pszPropVal);
        } else if (stmt->d.s_propfilt.prop.id == PROP_LOCAL_VAR) {
            DBGPRINTF("Filter: check for local var '%s' (value '%s') ", stmt->d.s_propfilt.prop.name, pszPropVal);
        } else if (stmt->d.s_propfilt.prop.id == PROP_GLOBAL_VAR) {
            DBGPRINTF("Filter: check for global var '%s' (value '%s') ", stmt->d.s_propfilt.prop.name, pszPropVal);
        } else {
            DBGPRINTF("Filter: check for property '%s' (value '%s') ", propIDToName(stmt->d.s_propfilt.prop.id),
                      pszPropVal);
        }
        if (stmt->d.s_propfilt.isNegated) DBGPRINTF("NOT ");
        if (stmt->d.s_propfilt.operation == FIOP_ISEMPTY) {
            DBGPRINTF("%s : %s\n", getFIOPName(stmt->d.s_propfilt.operation), bRet ? "TRUE" : "FALSE");
        } else {
            DBGPRINTF("%s '%s': %s\n", getFIOPName(stmt->d.s_propfilt.operation),
                      rsCStrGetSzStrNoNULL(stmt->d.s_propfilt.pCSCompValue), bRet ? "TRUE" : "FALSE");
        }
    }

    /* cleanup */
    if (pbMustBeFreed) free(pszPropVal);
done:
    return bRet;
}

static rsRetVal execPROPFILT(struct cnfstmt *stmt, smsg_t *pMsg, wti_t *pWti) {
    sbool bRet;
    DEFiRet;

    bRet = evalPROPFILT(stmt, pMsg);
    DBGPRINTF("PROPFILT condition result is %d\n", bRet);
    if (bRet) CHKiRet(scriptExec(stmt->d.s_propfilt.t_then, pMsg, pWti));
finalize_it:
    RETiRet;
}

static rsRetVal ATTR_NONNULL() execReloadLookupTable(struct cnfstmt *stmt) {
    assert(stmt != NULL);
    lookup_ref_t *t;
    DEFiRet;
 ...
 */
static parserList_t *GetParserList(rsconf_t *conf, smsg_t *pMsg) {
...
static void ATTR_NONNULL(1) addScript(ruleset_t *const pThis, struct cnfstmt *const script) {
 ...
 */
static ruleset_t *GetCurrent(rsconf_t *conf) {
 ...
 * is really much more natural to return the pointer directly.
 */
static qqueue_t *GetRulesetQueue(ruleset_t *pThis) {
 ...
 */
static rsRetVal SetDefaultRuleset(rsconf_t *conf, uchar *pszName) {
    ruleset_t *pRuleset;
    DEFiRet;
    assert(pszName != NULL);
...
static rsRetVal SetCurrRuleset(rsconf_t *conf, uchar *pszName) {
    ruleset_t *pRuleset;
    DEFiRet;
    assert(pszName != NULL);
 ...
 */
static rsRetVal rulesetConstructFinalize(rsconf_t *conf, ruleset_t *pThis) {
    uchar *keyName;
    DEFiRet;
    ISOBJ_TYPE_assert(pThis, ruleset);
    ...
    if (conf->rulesets.pDflt == NULL) conf->rulesets.pDflt = pThis;
 ...
 */
static rsRetVal destructAllActions(rsconf_t *conf) {
    DEFiRet;
    ...
    CHKiRet(llInit(&(conf->rulesets.llRulesets), rulesetDestructForLinkedList, rulesetKeyDestruct,
 ...
 */
static rsRetVal debugPrintAll(rsconf_t *conf) {
    DEFiRet;
...
static void rulesetOptimize(ruleset_t *pRuleset) {
    if (Debug) {
 ...
 */
static rsRetVal doRulesetCreateQueue(rsconf_t *conf, int *pNewVal) {
    uchar *rsname;
    DEFiRet;

    if (conf->rulesets.pCurr == NULL) {
        LogError(0, RS_RET_NO_CURR_RULESET,
                 "error: currently no specific ruleset specified, thus a "
                 "queue can not be added to it");
        ABORT_FINALIZE(RS_RET_NO_CURR_RULESET);
    }

    if (conf->rulesets.pCurr->pQueue != NULL) {
        LogError(0, RS_RET_RULES_QUEUE_EXISTS,
                 "error: ruleset already has a main queue, can not "
                 "add another one");
        ABORT_FINALIZE(RS_RET_RULES_QUEUE_EXISTS);
    }

    if (pNewVal == 0) FINALIZE; /* if it is turned off, we do not need to change anything ;) */
...
static rsRetVal rulesetCreateQueue(void __attribute__((unused)) * pVal, int *pNewVal) {
 ...
 * rgerhards, 2009-11-04
 */
static rsRetVal doRulesetAddParser(ruleset_t *pRuleset, uchar *pName) {
    parser_t *pParser;
    DEFiRet;
    ...
    if (iRet == RS_RET_PARSER_NOT_FOUND) {
        LogError(0, RS_RET_PARSER_NOT_FOUND,
                 "error: parser '%s' unknown at this time "
                 "(maybe defined too late in rsyslog.conf?)",
                 pName);
        ABORT_FINALIZE(RS_RET_NO_CURR_RULESET);
...
static rsRetVal rulesetAddParser(void __attribute__((unused)) * pVal, uchar *pName) {
...
rsRetVal rulesetProcessCnf(struct cnfobj *o) {
    ...
    if (localRet == RS_RET_OK) {
        LogError(0, RS_RET_RULESET_EXISTS, "error: ruleset '%s' specified more than once", rsName);
