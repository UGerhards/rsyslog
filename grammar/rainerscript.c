 ...
 *
 * Copyright 2011-2025 Rainer Gerhards and Others.
 *
...
PRAGMA_IGNORE_Wswitch_enum
    ...
    DEFobjCurrIf(obj) DEFobjCurrIf(regexp) DEFobjCurrIf(datetime) DEFobjCurrIf(parser)
...
static struct cnfparamdescr incpdescr[] = {
    {"file", eCmdHdlrString, 0}, {"text", eCmdHdlrString, 0}, {"mode", eCmdHdlrGetWord, 0}};
static struct cnfparamblk incpblk = {CNFPARAMBLK_VERSION, sizeof(incpdescr) / sizeof(struct cnfparamdescr), incpdescr};
 ...
 *       grammar.
 */
static const char *tokenToString(const int token) {
    const char *tokstr;
    ...
    switch (token) {
        ...
        case BEGIN_ACTION:
            ...
            break;
        case BEGIN_PROPERTY:
            tokstr = "BEGIN_PROPERTY";
            break;
        case BEGIN_CONSTANT:
            tokstr = "BEGIN_CONSTANT";
            break;
        case BEGIN_TPL:
            tokstr = "BEGIN_TPL";
            break;
        case BEGIN_RULESET:
            tokstr = "BEGIN_RULESET";
            break;
        case STOP:
            tokstr = "STOP";
            break;
        ...
        case CALL:
            ...
            break;
        case LEGACY_ACTION:
            tokstr = "LEGACY_ACTION";
            break;
        case LEGACY_RULESET:
            tokstr = "LEGACY_RULESET";
            break;
        case PRIFILT:
            tokstr = "PRIFILT";
            break;
        ...
        case CMP_CONTAINS:
            ...
            break;
        case CMP_CONTAINSI:
            tokstr = "CMP_CONTAINSI";
            break;
        case CMP_STARTSWITH:
            tokstr = "CMP_STARTSWITH";
            break;
        case CMP_STARTSWITHI:
            tokstr = "CMP_STARTSWITHI";
            break;
        case CMP_ENDSWITH:
            tokstr = "CMP_ENDSWITH";
            break;
        ...
        case S_FUNC_EXISTS:
            ...
            break;
        default:
            snprintf(tokbuf, sizeof(tokbuf), "%c[%d]", token, token);
            tokstr = tokbuf;
            break;
    }
    return tokstr;
...
const char *getFIOPName(const unsigned iFIOP) {
    const char *pRet;
    switch (iFIOP) {
        case FIOP_CONTAINS:
            pRet = "contains";
...
const char *cnfobjType2str(const enum cnfobjType ot) {
    switch (ot) {
        case CNFOBJ_ACTION:
            return "action";
            break;
 ...
 * of the action part.
 */
static rsRetVal DecodePropFilter(uchar *pline, struct cnfstmt *stmt) {
    ...
    CHKiRet(msgPropDescrFill(&stmt->d.s_propfilt.prop, cstrGetSzStrNoNULL(pCSPropName), cstrLen(pCSPropName)));
    ...
    if (!rsCStrOffsetSzStrCmp(pCSCompOp, iOffset, (uchar *)"contains", 8)) {
        ...
        parser_errmsg("error: invalid compare operation '%s'", (char *)rsCStrGetSzStrNoNULL(pCSCompOp));
...
finalize_it:
    if (pPars != NULL) rsParsDestruct(pPars);
    if (pCSCompOp != NULL) rsCStrDestruct(&pCSCompOp);
    if (pCSPropName != NULL) cstrDestruct(&pCSPropName);
    RETiRet;
}

static void prifiltInvert(struct funcData_prifilt *__restrict__ const prifilt) {
    int i;
 ...
 * CMP_GE, CMP_NE.
 */
static void prifiltSetSeverity(struct funcData_prifilt *prifilt, int sev, int mode) {
    ...
    if (mode == CMP_NE) prifiltInvert(prifilt);
}
 ...
 */
static void prifiltSetFacility(struct funcData_prifilt *__restrict__ const prifilt, const int fac, const int mode) {
    int i;
    ...
    switch (mode) {
        ...
        case CMP_NE:
            ...
            break;
        case CMP_LT:
            for (i = 0; i < fac; ++i) prifilt->pmask[i] = TABLE_ALLPRI;
            break;
        case CMP_LE:
            for (i = 0; i < fac + 1; ++i) prifilt->pmask[i] = TABLE_ALLPRI;
            break;
        case CMP_GE:
            for (i = fac; i < LOG_NFACILITIES + 1; ++i) prifilt->pmask[i] = TABLE_ALLPRI;
            break;
        case CMP_GT:
            for (i = fac + 1; i < LOG_NFACILITIES + 1; ++i) prifilt->pmask[i] = TABLE_ALLPRI;
            break;
        default:
            break;
    }
 ...
 */
static void prifiltCombine(struct funcData_prifilt *__restrict__ const prifilt,
...
void readConfFile(FILE *const fp, es_str_t **str) {
    ...
    *str = es_newStr(4096);
    ...
    while (fgets(ln, sizeof(ln), fp) != NULL) {
        ...
        if (bContLine) {
            for (start = 0; start < len && isspace(ln[start]); ++start) /* JUST SCAN */
                ;
        } else {
            start = 0;
        }
        for (i = len - 1; i >= start && isspace(ln[i]); --i) /* JUST SCAN */
            ;
        if (i >= 0) {
            if (ln[i] == '\\') {
                --i;
...
static int qs_arrcmp(const void *s1, const void *s2) {
    return es_strcmp(*((es_str_t **)s1), *((es_str_t **)s2));
}


struct objlst *objlstNew(struct cnfobj *o) {
    struct objlst *lst;
...
struct nvlst *ATTR_NONNULL(1) nvlstNewStr(es_str_t *const value) {
    struct nvlst *lst;
...
struct nvlst *ATTR_NONNULL(1) nvlstNewStrBackticks(es_str_t *const value) {
    ...
    if (strncmp(param, "echo $", sizeof("echo $") - 1) != 0) {
        parser_errmsg(
            "invalid backtick parameter `%s` currently "
    ...
    while (lst != NULL) {
        ...
        switch (lst->val.datatype) {
            ...
            case 'S':
                ...
                break;
            default:
                dbgprintf("nvlstPrint: unknown type '%s'\n", tokenToString(lst->val.datatype));
                break;
        }
        free(name);
        lst = lst->next;
...
struct nvlst *nvlstFindName(struct nvlst *lst, es_str_t *name) {
    while (lst != NULL && es_strcmp(lst->name, name)) lst = lst->next;
    return lst;
}
 ...
 */
static struct nvlst *nvlstFindNameCStr(struct nvlst *lst, const char *const __restrict__ name) {
    es_size_t lenName = strlen(name);
    while (lst != NULL && es_strcasebufcmp(lst->name, (uchar *)name, lenName)) lst = lst->next;
    return lst;
}
 ...
 */
static void nvlstChkDupes(struct nvlst *lst) {
    char *cstr;
...
static int doGetSize(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    unsigned char *c;
    es_size_t i;
    long long n;
    int r;
    ...
    if (i < es_strlen(valnode->val.d.estr)) {
        ...
        switch (*c) {
            ...
            case 'g':
                ...
                break;
            case 't':
                n *= (int64)1024 * 1024 * 1024 * 1024;
                break; /* tera */
            case 'p':
                n *= (int64)1024 * 1024 * 1024 * 1024 * 1024;
                break; /* peta */
            case 'e':
                n *= (int64)1024 * 1024 * 1024 * 1024 * 1024 * 1024;
                break; /* exa */
            ...
            case 'T':
                n *= (int64)1000 * 1000000000;
                break; /* tera */
            case 'P':
                n *= (int64)1000000 * 1000000000;
                break; /* peta */
            case 'E':
                n *= (int64)1000000000 * 1000000000;
                break; /* exa */
            default:
                --i;
                break; /* indicates error */
        }
    }
    if (i == es_strlen(valnode->val.d.estr)) {
        val->val.datatype = 'N';
        ...
        parser_errmsg("parameter '%s' does not contain a valid size", param->name);
        r = 0;
    }
    return r;
}
...
static int doGetBinary(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    int r = 1;
    val->val.datatype = 'N';
    if (!es_strbufcmp(valnode->val.d.estr, (unsigned char *)"on", 2)) {
        val->val.d.n = 1;
        ...
        parser_errmsg(
            "parameter '%s' must be \"on\" or \"off\" but "
            "is neither. Results unpredictable.",
            param->name);
        val->val.d.n = 0;
        r = 0;
    }
...
static int doGetQueueType(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    char *cstr;
    int r = 1;
 ...
 * starting with '0'.
 */
static int doGetFileCreateMode(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    int fmtOK = 0;
    char *cstr;
    uchar *c;
    ...
    if (len_val >= 4) {
        c = es_getBufAddr(valnode->val.d.estr);
        if ((c[0] == '0') && (c[1] >= '0' && c[1] <= '7') && (c[2] >= '0' && c[2] <= '7') &&
            (c[3] >= '0' && c[3] <= '7')) {
            if (len_val == 5) {
                if (c[4] >= '0' && c[4] <= '7') {
                    fmtOK = 1;
...
static int doGetGID(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    char *cstr;
    int r;
    ...
    if (resultBuf == NULL) {
        if (e != 0) {
            if (loadConf->globals.abortOnIDResolutionFail) {
                fprintf(stderr,
                        "parameter '%s': error to "
                        "obtaining group id for '%s'",
                        param->name, cstr);
                exit(1); /* good exit */
            } else {
                LogError(e, RS_RET_ERR,
                         "parameter '%s': error to "
                         "obtaining group id for '%s'",
                         param->name, cstr);
            }
        ...
        parser_errmsg(
            "parameter '%s': ID for group %s could not "
            "be found",
            param->name, cstr);
        r = 0;
    } else {
...
static int doGetUID(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    char *cstr;
    int r;
    ...
    if (resultBuf == NULL) {
        ...
        if (loadConf->globals.abortOnIDResolutionFail) {
            fprintf(stderr,
                    "parameter '%s': ID for user '%s' could not "
                    "be found: %s",
                    param->name, cstr, errStr);
            exit(1); /* good exit */
        } else {
            LogError(err_no, RS_RET_ERR,
                     "parameter '%s': ID for user '%s' could not "
                     "be found: %s",
                     param->name, cstr, errStr);
            parser_errmsg(
 ...
 */
static int doGetInt(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    long long n;
    int bSuccess;

    n = es_str2num(valnode->val.d.estr, &bSuccess);
    if (!bSuccess) {
        parser_errmsg("parameter '%s' is not a proper number", param->name);
    }
    val->val.datatype = 'N';
    val->val.d.n = n;
    return bSuccess;
...
static int doGetNonNegInt(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    int bSuccess;

    if ((bSuccess = doGetInt(valnode, param, val))) {
        if (val->val.d.n < 0) {
            parser_errmsg("parameter '%s' cannot be less than zero (was %lld)", param->name, val->val.d.n);
            bSuccess = 0;
        }
    }
    return bSuccess;
...
static int doGetPositiveInt(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    int bSuccess;

    if ((bSuccess = doGetInt(valnode, param, val))) {
        if (val->val.d.n < 1) {
            parser_errmsg("parameter '%s' cannot be less than one (was %lld)", param->name, val->val.d.n);
            bSuccess = 0;
        }
    }
    return bSuccess;
...
static int doGetWord(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    es_size_t i;
    int r = 1;
    unsigned char *c;
    ...
    if (i != es_strlen(valnode->val.d.estr)) {
        parser_errmsg(
            "parameter '%s' contains whitespace, which is not "
            "permitted",
            param->name);
        r = 0;
    }
...
static int doGetArray(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    int r = 1;
    ...
    switch (valnode->val.datatype) {
        ...
        default:
            parser_errmsg(
                "parameter '%s' must be an array, but is a "
                "different datatype",
                param->name);
            r = 0;
            break;
...
static int doGetChar(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    int r = 1;
    if (es_strlen(valnode->val.d.estr) != 1) {
        parser_errmsg(
            "parameter '%s' must contain exactly one character "
 ...
 */
static int nvlstGetParam(struct nvlst *valnode, struct cnfparamdescr *param, struct cnfparamvals *val) {
    uchar *cstr;
    int r;
    ...
    if (valnode->val.datatype != 'S' && param->type != eCmdHdlrArray) {
        parser_errmsg(
            "parameter '%s' is not a string, which is not "
            "permitted",
            param->name);
        r = 0;
        goto done;
    ...
    switch (param->type) {
        ...
        case eCmdHdlrGoneAway:
            parser_errmsg("parameter '%s' is no longer supported", param->name);
            r = 1; /* this *is* valid! */
            break;
        default:
...
struct cnfparamvals *ATTR_NONNULL(2)
    nvlstGetParams(struct nvlst *lst, struct cnfparamblk *params, struct cnfparamvals *vals) {
    ...
    if (params->version != CNFPARAMBLK_VERSION) {
        DBGPRINTF(
            "nvlstGetParams: invalid param block version "
            "%d, expected %d\n",
    ...
    if (vals == NULL) {
        bValsWasNULL = 1;
        if ((vals = calloc(params->nParams, sizeof(struct cnfparamvals))) == NULL) return NULL;
    } else {
        bValsWasNULL = 0;
    }
    ...
    for (i = 0; i < params->nParams; ++i) {
        ...
        if ((valnode = nvlstFindNameCStr(lst, param->name)) == NULL) {
            if (param->flags & CNFPARAM_REQUIRED) {
                parser_errmsg(
                    "parameter '%s' required but not specified - "
                    "fix config",
                    param->name);
                bInError = 1;
            }
        ...
        if (param->flags & CNFPARAM_DEPRECATED) {
            parser_errmsg(
                "parameter '%s' deprecated but accepted, consider "
        ...
        if (vals[i].bUsed) {
            parser_errmsg(
                ...
                "one instance is ignored. Fix config",
                param->name);
            continue;
        }
    ...
    if (bInError) {
        if (bValsWasNULL) cnfparamvalsDestruct(vals, params);
        vals = NULL;
    }
        ...
        if (vals[i].bUsed) {
            ...
            switch (vals[i].val.datatype) {
                ...
                case 'N':
                    ...
                    break;
                default:
                    dbgprintf("(unsupported datatype %c)", vals[i].val.datatype);
            }
        } else {
            dbgprintf("(unset)");
        }
...
static int64_t str2num(es_str_t *s, int *bSuccess) {
    size_t i;
    int neg;
    int64_t num = 0;
    ...
    num *= neg;
    if (bSuccess != NULL) *bSuccess = (i == s->lenStr) ? 1 : 0;
done:
    return num;
}
...
static void varFreeMembersSelectively(const struct svar *r, const int skipMask) {
    if (r->datatype == 'J') {
...
static rsRetVal doExtractFieldByChar(uchar *str, uchar delim, const int matchnbr, uchar **resstr) {
    int iCurrFld;
    int allocLen;
    int iLen;
    uchar *pBuf;
    ...
    while (*pFld && iCurrFld < matchnbr) {
        ...
        while (*pFld && (uchar)*pFld != delim) ++pFld; /* skip to field terminator */
        if (*pFld == delim) {
            ++pFld; /* eat it */
            ++iCurrFld;
        }
    ...
    if (iCurrFld == matchnbr) {
        ...
        pFldEnd = pFld;
        while (*pFldEnd && *pFldEnd != delim) ++pFldEnd;
...
static rsRetVal doExtractFieldByStr(
    uchar *str, char *delim, const rs_size_t lenDelim, const int matchnbr, uchar **resstr) {
    int iCurrFld;
    int iLen;
    uchar *pBuf;
    uchar *pFld;
    ...
    if (str == NULL || delim == NULL) ABORT_FINALIZE(RS_RET_FIELD_NOT_FOUND);
...
static void doFunc_re_extract(struct cnffunc *func, struct svar *ret, void *usrptr, wti_t *const pWti) {
    size_t submatchnbr;
    short matchnbr;
    ...
    while (!bFound) {
        int iREstat;
        iREstat = regexp.regexec(func->funcdata, (char *)(str + iOffs), submatchnbr + 1, pmatch, 0);
    ...
    if (!bFound) {
        ...
        estr = es_newStrFromBuf(str + iOffs + pmatch[submatchnbr].rm_so, iLenBuf);
    }
...
static es_str_t *doFuncReplace(struct svar *__restrict__ const operandVal,
                               struct svar *__restrict__ const findVal,
...
static void ATTR_NONNULL() doFunc_parse_json(struct cnffunc *__restrict__ const func,
                                             struct svar *__restrict__ const ret,
                                             void *const usrptr,
                                             wti_t *const pWti) {
...
static void ATTR_NONNULL() doFunc_get_property(struct cnffunc *__restrict__ const func,
                                               struct svar *__restrict__ const ret,
                                               void *const usrptr,
                                               wti_t *const pWti) {
    ...
    switch (json_object_get_type(json)) {
        ...
        default:
            LogError(0, RS_RET_INTERNAL_ERROR, "Warning - unhandled json type(%d) !!!!\n", json_object_get_type(json));
            retVal = RS_SCRIPT_EINVAL;
            break;
    }
...
static void ATTR_NONNULL() doFunct_RandomGen(struct cnffunc *__restrict__ const func,
    ...
    if (!success) {
        DBGPRINTF(
            "rainerscript: random(max) didn't get a valid 'max' limit, defaulting random-number "
            "value to 0");
        retVal = 0;
        goto done;
    }
...
static void ATTR_NONNULL() doFunct_LTrim(struct cnffunc *__restrict__ const func,
                                         struct svar *__restrict__ const ret,
                                         void *__restrict__ const usrptr,
                                         wti_t *__restrict__ const pWti) {
    struct svar srcVal;
...
static void ATTR_NONNULL() doFunct_RTrim(struct cnffunc *__restrict__ const func,
                                         struct svar *__restrict__ const ret,
                                         void *__restrict__ const usrptr,
                                         wti_t *__restrict__ const pWti) {
    struct svar srcVal;
...
static void ATTR_NONNULL() doFunct_Getenv(struct cnffunc *__restrict__ const func,
                                          struct svar *__restrict__ const ret,
                                          void *__restrict__ const usrptr,
                                          wti_t *__restrict__ const pWti) {
...
static void ATTR_NONNULL() doFunct_ToLower(struct cnffunc *__restrict__ const func,
                                           struct svar *__restrict__ const ret,
                                           void *__restrict__ const usrptr,
                                           wti_t *__restrict__ const pWti) {
    struct svar srcVal;
...
static void ATTR_NONNULL() doFunct_CNum(struct cnffunc *__restrict__ const func,
                                        struct svar *__restrict__ const ret,
                                        void *__restrict__ const usrptr,
                                        wti_t *__restrict__ const pWti) {
    struct svar srcVal;
    ...
    if (func->expr[0]->nodetype == 'N') {
        ...
        ret->d.n = es_str2num(((struct cnfstringval *)func->expr[0])->estr, NULL);
    } else {
        cnfexprEval(func->expr[0], &srcVal, usrptr, pWti);
...
static void ATTR_NONNULL() doFunct_Ipv42num(struct cnffunc *__restrict__ const func,
                                            struct svar *__restrict__ const ret,
                                            void *__restrict__ const usrptr,
                                            wti_t *__restrict__ const pWti) {
    struct svar srcVal;
...
static void ATTR_NONNULL() doFunct_Int2Hex(struct cnffunc *__restrict__ const func,
                                           struct svar *__restrict__ const ret,
                                           void *__restrict__ const usrptr,
                                           wti_t *__restrict__ const pWti) {
    struct svar srcVal;
...
static void ATTR_NONNULL() doFunct_Replace(struct cnffunc *__restrict__ const func,
                                           struct svar *__restrict__ const ret,
                                           void *__restrict__ const usrptr,
                                           wti_t *__restrict__ const pWti) {
...
static void ATTR_NONNULL() doFunct_Wrap(struct cnffunc *__restrict__ const func,
                                        struct svar *__restrict__ const ret,
                                        void *__restrict__ const usrptr,
                                        wti_t *__restrict__ const pWti) {
...
static void ATTR_NONNULL() doFunct_StrLen(struct cnffunc *__restrict__ const func,
                                          struct svar *__restrict__ const ret,
                                          void *__restrict__ const usrptr,
                                          wti_t *__restrict__ const pWti) {
    struct svar srcVal;
...
static void ATTR_NONNULL()
    doFunct_Substring(struct cnffunc *__restrict__ const func,
                      struct svar *__restrict__ const ret,
                      void *__restrict__ const usrptr,
                      wti_t *__restrict__ const pWti) {  // TODO: generalize parameter getter? jgerhards, 2018-02-26
    int bMustFree;
    struct svar srcVal[3];
...
static void ATTR_NONNULL() doFunct_Field(struct cnffunc *__restrict__ const func,
                                         struct svar *__restrict__ const ret,
                                         void *__restrict__ const usrptr,
                                         wti_t *__restrict__ const pWti) {
    struct svar srcVal[3];
    ...
    if (srcVal[1].datatype == 'S') {
        ...
        localRet = doExtractFieldByStr((uchar *)str, delimstr, es_strlen(srcVal[1].d.estr), matchnbr, &resStr);
        free(delimstr);
    } else {
    ...
    if (localRet == RS_RET_OK) {
        ...
        ret->d.estr = es_newStrFromCStr("***FIELD NOT FOUND***", sizeof("***FIELD NOT FOUND***") - 1);
    } else {
        ret->d.estr =
            es_newStrFromCStr("***ERROR in field() FUNCTION***", sizeof("***ERROR in field() FUNCTION***") - 1);
    }
    ret->datatype = 'S';
    if (bMustFree) free(str);
    varFreeMembers(&srcVal[0]);
...
static void ATTR_NONNULL() doFunct_Prifilt(struct cnffunc *__restrict__ const func,
                                           struct svar *__restrict__ const ret,
                                           void *__restrict__ const usrptr,
                                           wti_t *const pWti __attribute__((unused))) {
    ...
    if ((pPrifilt->pmask[((smsg_t *)usrptr)->iFacility] == TABLE_NOPRI) ||
        ((pPrifilt->pmask[((smsg_t *)usrptr)->iFacility] & (1 << ((smsg_t *)usrptr)->iSeverity)) == 0))
        ret->d.n = 0;
    else
        ret->d.n = 1;
    ret->datatype = 'N';
...
static void ATTR_NONNULL() doFunct_Lookup(struct cnffunc *__restrict__ const func,
                                          struct svar *__restrict__ const ret,
                                          void *__restrict__ const usrptr,
                                          wti_t *__restrict__ const pWti) {
    struct svar srcVal;
...
static void ATTR_NONNULL() doFunct_DynInc(struct cnffunc *__restrict__ const func,
                                          struct svar *__restrict__ const ret,
                                          void *__restrict__ const usrptr,
                                          wti_t *__restrict__ const pWti) {
    struct svar srcVal;
...
static void ATTR_NONNULL() doFunct_FormatTime(struct cnffunc *__restrict__ const func,
                                              struct svar *__restrict__ const ret,
                                              void *__restrict__ const usrptr,
                                              wti_t *__restrict__ const pWti) {
    struct svar srcVal[2];
    ...
    if (sizeof(time_t) == sizeof(int)) {
        if (unixtime < INT_MIN || unixtime > INT_MAX) {
            LogMsg(0, RS_RET_VAL_OUT_OF_RANGE, LOG_WARNING,
 ...
 * im - "Incoming" Month
 */
static int estimateYear(int cy, int cm, int im) {
    im += 12;

    if ((im - cm) == 1) {
        if (cm == 12 && im == 13) return cy + 1;
    }

    if ((im - cm) > 13) return cy - 1;
    ...
    if (objUse(datetime, CORE_COMPONENT) == RS_RET_OK) {
        ...
        for (int i = 0; i < numFormats; i++) {
            ...
            if (f == DATE_RFC3339) {
                ...
                if (datetime.ParseTIMESTAMP3164(&s, (uchar **)&pszTS, &len, NO_PARSE3164_TZSTRING,
                                                NO_PERMIT_YEAR_AFTER_TIME) == RS_RET_OK) {
...
static void ATTR_NONNULL() doFunct_IsTime(struct cnffunc *__restrict__ const func,
                                          struct svar *__restrict__ const ret,
                                          void *__restrict__ const usrptr,
                                          wti_t *__restrict__ const pWti) {
    struct svar srcVal[2];
...
static void ATTR_NONNULL() doFunct_ScriptError(struct cnffunc *const func __attribute__((unused)),
                                               struct svar *__restrict__ const ret,
...
static void ATTR_NONNULL() doFunct_PreviousActionSuspended(struct cnffunc *const func __attribute__((unused)),
                                                           struct svar *__restrict__ const ret,
                                                           void *const usrptr __attribute__((unused)),
                                                           wti_t *__restrict__ const pWti) {
    ret->datatype = 'N';
...
static void ATTR_NONNULL() doFunct_Reparse(struct cnffunc *const func __attribute__((unused)),
    ...
    if (localRet == RS_RET_OK) {
        ...
        DBGPRINTF("rainerscript: reparse() failed with %d\n", localRet);
        wtiSetScriptErrno(pWti, RS_SCRIPT_EINVAL);
    }
}

static void ATTR_NONNULL() doFunct_num2ipv4(struct cnffunc *__restrict__ const func,
                                            struct svar *__restrict__ const ret,
                                            void *__restrict__ const usrptr,
                                            wti_t *__restrict__ const pWti) {
    struct svar srcVal;
    ...
    if (num < 0 || num > 4294967295) {
        DBGPRINTF(
            "rainerscript: (num2ipv4) invalid number(too big/negative); does "
 ...
 */
static void ATTR_NONNULL() doFuncCall(struct cnffunc *__restrict__ const func,
                                      struct svar *__restrict__ const ret,
                                      void *__restrict__ const usrptr,
                                      wti_t *__restrict__ const pWti) {
    if (Debug) {
...
static int ATTR_NONNULL()
    ...
    if (fexists->prop.id == PROP_CEE || fexists->prop.id == PROP_LOCAL_VAR || fexists->prop.id == PROP_GLOBAL_VAR) {
 ...
 */
static int evalStrArrayCmp(es_str_t *const estr_l, const struct cnfarray *__restrict__ const ar, const int cmpop) {
    int i;
    int r = 0;
    es_str_t **res;
    if (cmpop == CMP_EQ) {
        ...
        for (i = 0; (r == 0) && (i < ar->nmemb); ++i) {
            switch (cmpop) {
                ...
                case CMP_STARTSWITHI:
                    ...
                    break;
                case CMP_ENDSWITH: {
                    const size_t len_l = es_strlen(estr_l);
                    const size_t len_suffix = es_strlen(ar->arr[i]);
                    r = len_l >= len_suffix && (memcmp(es_getBufAddr(estr_l) + len_l - len_suffix,
                                                       es_getBufAddr(ar->arr[i]), len_suffix) == 0);
                } break;
                case CMP_CONTAINS:
 ...
 */
void ATTR_NONNULL() cnfexprEval(const struct cnfexpr *__restrict__ const expr,
                                struct svar *__restrict__ const ret,
                                void *__restrict__ const usrptr,
                                wti_t *__restrict__ const pWti) {
    struct svar r, l; /* memory for subexpression results */
    ...
    switch (expr->nodetype) {
        ...
        case CMP_ENDSWITH:
            ...
            if (expr->r->nodetype == 'A') {
                ...
                ret->d.n =
                    estr_l->lenStr >= estr_r->lenStr && (memcmp(es_getBufAddr(estr_l) + estr_l->lenStr - estr_r->lenStr,
                                                                es_getBufAddr(estr_r), estr_r->lenStr) == 0);
            }
            FREE_TWO_STRINGS;
            break;
        case CMP_STARTSWITHI:
...
static rsRetVal initFunc_dyn_stats(struct cnffunc *func) {
    uchar *cstr = NULL;
    DEFiRet;
    ...
    if (func->expr[0]->nodetype != 'S') {
        parser_errmsg(
            "dyn-stats bucket-name (param 1) of dyn-stats manipulating "
...
static rsRetVal initFunc_perctile_obs(struct cnffunc *func) {
    uchar *cstr = NULL;
    DEFiRet;
    ...
    if (func->expr[0]->nodetype != 'S') {
        parser_errmsg(
            "percentile-stats bucket-name (param 1) of perctile-stats manipulating "
...
static void ATTR_NONNULL() doFunc_percentile_obs(struct cnffunc *__restrict__ const func,
                                                 struct svar *__restrict__ const ret,
                                                 void *__restrict__ const usrptr,
                                                 wti_t *__restrict__ const pWti) {
    uchar *cstr = NULL;
...
static rsRetVal initFunc_re_match_generic(struct cnffunc *const func, const unsigned flags) {
    rsRetVal localRet;
    char *regex = NULL;
    regex_t *re;
    DEFiRet;
...
static rsRetVal initFunc_re_match(struct cnffunc *func) {
    return initFunc_re_match_generic(func, 0);
}

static rsRetVal initFunc_re_match_i(struct cnffunc *func) {
    return initFunc_re_match_generic(func, REG_ICASE);
}

static rsRetVal initFunc_exec_template(struct cnffunc *func) {
    char *tplName = NULL;
    DEFiRet;
...
static rsRetVal initFunc_prifilt(struct cnffunc *func) {
...
static rsRetVal resolveLookupTable(struct cnffunc *func) {
    uchar *cstr = NULL;
    char *fn_name = NULL;
    DEFiRet;
...
static rscriptFuncPtr ATTR_NONNULL() extractFuncPtr(const struct scriptFunct *const funct, const unsigned int nParams) {
    ...
    if (funct->minParams == funct->maxParams) {
        if (nParams == funct->maxParams) {
            ...
            parser_errmsg("number of parameters for %s() must be %hu but is %d.", funct->fname, funct->maxParams,
                          nParams);
        }
    } else {
        if (nParams < funct->minParams) {
            parser_errmsg("number of parameters for %s() must be at least %hu but is %d.", funct->fname,
                          funct->minParams, nParams);
        } else if (nParams > funct->maxParams) {
            parser_errmsg("number of parameters for %s() must be at most %hu but is %d.", funct->fname,
                          funct->maxParams, nParams);
        } else {
            retPtr = funct->fPtr;
        }
    }
...
static struct scriptFunct *ATTR_NONNULL() searchFunctArray(const char *const fname, struct scriptFunct *functArray) {
...
static struct scriptFunct *ATTR_NONNULL() searchModList(const char *const fname) {
...
static void cnffuncDestruct(struct cnffunc *func) {
    unsigned short i;
 ...
 */
void cnfexprDestruct(struct cnfexpr *__restrict__ const expr) {
    if (expr == NULL) {
 ...
 * important.
 */
int cnfexprEvalBool(struct cnfexpr *__restrict__ const expr, void *__restrict__ const usrptr, wti_t *const pWti) {
    int convok;
    struct svar ret;
...
struct json_object *cnfexprEvalCollection(struct cnfexpr *__restrict__ const expr,
                                          void *__restrict__ const usrptr,
                                          wti_t *const pWti) {
    struct svar ret;
    void *retptr;
...
static void doIndent(int indent) {
    int i;
    for (i = 0; i < indent; ++i) dbgprintf("  ");
}

static void pmaskPrint(uchar *pmask, int indent) {
    int i;
    doIndent(indent);
    dbgprintf("pmask: ");
...
void cnfexprPrint(struct cnfexpr *expr, int indent) {
    ...
    switch (expr->nodetype) {
        ...
        case '%':
        case 'M':
            if (expr->l != NULL) cnfexprPrint(expr->l, indent + 1);
            doIndent(indent);
            dbgprintf("%c\n", (char)expr->nodetype);
            cnfexprPrint(expr->r, indent + 1);
            break;
...
void cnfstmtPrintOnly(struct cnfstmt *stmt, int indent, sbool subtree) {
    ...
    switch (stmt->nodetype) {
        ...
        case S_CALL:
            ...
            doIndent(indent);
            dbgprintf("CALL [%s, queue:%d]\n", cstr, stmt->d.s_call.ruleset == NULL ? 0 : 1);
            free(cstr);
            break;
        case S_CALL_INDIRECT:
            doIndent(indent);
            dbgprintf("CALL_INDIRECT\n");
            cnfexprPrint(stmt->d.s_call_ind.expr, indent + 1);
            break;
        case S_ACT:
            doIndent(indent);
            dbgprintf("ACTION %d [%s:%s]\n", stmt->d.act->iActionNbr, modGetName(stmt->d.act->pMod), stmt->printable);
            break;
        case S_IF:
            doIndent(indent);
            dbgprintf("IF\n");
        ...
        case S_RELOAD_LOOKUP_TABLE:
            ...
            dbgprintf("RELOAD_LOOKUP_TABLE table(%s) (stub with '%s' on error)",
                      stmt->d.s_reload_lookup_table.table_name, stmt->d.s_reload_lookup_table.stub_value);
            break;
        case S_PRIFILT:
            doIndent(indent);
            dbgprintf("PRIFILT '%s'\n", stmt->printable);
        ...
        case S_PROPFILT:
            ...
            doIndent(indent);
            dbgprintf("\tOperation: ");
            if (stmt->d.s_propfilt.isNegated) dbgprintf("NOT ");
...
void cnfstmtPrint(struct cnfstmt *root, int indent) {
    struct cnfstmt *stmt;
...
struct cnfnumval *cnfnumvalNew(const long long val) {
...
struct cnfstringval *cnfstringvalNew(es_str_t *const estr) {
...
struct cnfstmt *cnfstmtNewSet(char *var, struct cnfexpr *expr, int force_reset) {
    ...
    if ((cnfstmt = cnfstmtNew(S_SET)) != NULL) {
        if (propNameToID((uchar *)var, &propid) == RS_RET_OK &&
            (propid == PROP_CEE || propid == PROP_LOCAL_VAR || propid == PROP_GLOBAL_VAR)) {
        ...
        switch (nParams) {
            case 2:
                ...
                if ((cnfstmt->d.s_reload_lookup_table.stub_value =
                    ...
                    parser_errmsg(
                        "statement ignored: reload_lookup_table statement "
            ...
            default:
                parser_errmsg(
                    "statement ignored: reload_lookup_table(table_name, optional:"
...
struct cnfstmt *cnfstmtNewAct(struct nvlst *lst) {
    ...
    if (localRet == RS_RET_OK_WARN) {
        ...
        parser_errmsg("errors occurred in file '%s' around line %d", cnfcurrfn, yylineno);
        cnfstmt->nodetype = S_NOP; /* disable action! */
        goto done;
    }
    snprintf(namebuf, sizeof(namebuf) - 1, "action(type=\"%s\" ...)", modGetName(cnfstmt->d.act->pMod));
...
struct cnfstmt *cnfstmtNewLegaAct(char *actline) {
    ...
    if (localRet != RS_RET_OK) {
        parser_errmsg("%s occurred in file '%s' around line %d", (localRet == RS_RET_OK_WARN) ? "warnings" : "errors",
                      cnfcurrfn, yylineno);
        if (localRet != RS_RET_OK_WARN) {
            cnfstmt->nodetype = S_NOP; /* disable action! */
            goto done;
 ...
 */
static int getConstNumber(struct cnfexpr *expr, long long *l, long long *r) {
    int ret = 0;
...
static void constFoldConcat(struct cnfexpr *expr) {
    es_str_t *estr;
    cnfexprOptimize(expr->l);
    cnfexprOptimize(expr->r);
 ...
 */
static struct cnfexpr *cnfexprOptimize_CMP_severity_facility(struct cnfexpr *expr) {
    struct cnffunc *func;

    if (expr->l->nodetype != 'V') FINALIZE;
    ...
    if (!strcmp("syslogseverity", ((struct cnfvar *)expr->l)->name)) {
        if (expr->r->nodetype == 'N') {
            ...
            if (sev >= 0 && sev <= 7) {
                ...
                parser_errmsg(
                    "invalid syslogseverity %d, expression will always "
                    "evaluate to FALSE",
                    sev);
            }
        }
        ...
        if (expr->r->nodetype == 'N') {
            ...
            if (fac >= 0 && fac <= 24) {
                ...
                parser_errmsg(
                    "invalid syslogfacility %d, expression will always "
                    "evaluate to FALSE",
                    fac);
            }
        }
 ...
 */
static struct cnfexpr *cnfexprOptimize_CMP_var(struct cnfexpr *expr) {
    struct cnffunc *func;
    ...
    if (!strcmp("syslogfacility-text", ((struct cnfvar *)expr->l)->name)) {
        if (expr->r->nodetype == 'S') {
            ...
            if (fac == -1) {
                parser_errmsg(
                    "invalid facility '%s', expression will always "
                    "evaluate to FALSE",
                    cstr);
            } else {
                ...
                if (expr->nodetype == CMP_NE) prifiltInvert(func->funcdata);
                cnfexprDestruct(expr);
                expr = (struct cnfexpr *)func;
            }
            free(cstr);
        ...
        if (expr->r->nodetype == 'S') {
            ...
            if (sev == -1) {
                parser_errmsg(
                    "invalid syslogseverity '%s', expression will always "
                    "evaluate to FALSE",
                    cstr);
            } else {
...
static struct cnfexpr *cnfexprOptimize_NOT(struct cnfexpr *expr) {
    struct cnffunc *func;
 ...
 */
static inline void cnfexprOptimize_CMPEQ_arr(struct cnfarray *arr) {
 ...
 * first non-NOP entry.
 */
static struct cnfstmt *removeNOPs(struct cnfstmt *const root) {
    ...
    while (stmt != NULL) {
        if (stmt->nodetype == S_NOP) {
            if (prevstmt != NULL) /* end chain, is rebuild if more non-NOPs follow */
                prevstmt->next = NULL;
            toDel = stmt;
            stmt = stmt->next;
            cnfstmtDestruct(toDel);
        } else {
            if (newRoot == NULL) newRoot = stmt;
...
static void cnfstmtOptimizeForeach(struct cnfstmt *stmt) {
...
static void cnfstmtOptimizeIf(struct cnfstmt *stmt) {
    ...
    if (stmt->d.s_if.expr->nodetype == 'F') {
        ...
        if (func->fPtr == doFunct_Prifilt) {
            ...
            memcpy(stmt->d.s_prifilt.pmask, prifilt->pmask, sizeof(prifilt->pmask));
            stmt->d.s_prifilt.t_then = t_then;
            stmt->d.s_prifilt.t_else = t_else;
            if (func->nParams == 0)
                stmt->printable = (uchar *)strdup("[Optimizer Result]");
            else
                stmt->printable = (uchar *)es_str2cstr(((struct cnfstringval *)func->expr[0])->estr, NULL);
            cnfexprDestruct(expr);
...
static void cnfstmtOptimizeAct(struct cnfstmt *stmt) {
    action_t *pAct;
...
static void cnfstmtOptimizePRIFilt(struct cnfstmt *stmt) {
    ...
    for (last = subroot; last->next != NULL; last = last->next) /* find last node in subtree */
        ;
    last->next = stmt->next;
...
static void cnfstmtOptimizeReloadLookupTable(struct cnfstmt *stmt) {
 ...
 */
static void cnfstmtOptimizeCall(struct cnfstmt *stmt) {
    ruleset_t *pRuleset;
    rsRetVal localRet;
    uchar *rsName;
    ...
    for (stmt = root; stmt != NULL; stmt = stmt->next) {
        ...
        switch (stmt->nodetype) {
            ...
            case S_CALL_INDIRECT:
                ...
                break;
            case S_STOP:
                if (stmt->next != NULL) parser_warnmsg("STOP is followed by unreachable statements!\n");
                break;
 ...
 */
static rscriptFuncPtr funcName2Ptr(char *const fname, const unsigned short nParams) {
...
/*version currently not used, might be needed later for versin check*/
{
    ...
    while (functArray[i].fname != NULL) {
        if (searchModList(functArray[i].fname) != NULL) {
            parser_errmsg("function %s defined multiple times, second time will be ignored", functArray[i].fname);
        }
        i++;
    }
...
struct cnffunc *cnffuncNew(es_str_t *fname, struct cnffparamlst *paramlst) {
    ...
    nParams = 0;
    for (param = paramlst; param != NULL; param = param->next) ++nParams;
    if ((func = malloc(sizeof(struct cnffunc) + (nParams * sizeof(struct cnfexp *)))) != NULL) {
        func->nodetype = 'F';
        func->fname = fname;
...
struct cnffunc *cnffuncNew_prifilt(int fac) {
    ...
    fac >>= 3;
    if (fac >= LOG_NFACILITIES + 1 || fac < 0) return NULL;
 ...
 */
int ATTR_NONNULL() cnfDoInclude(const char *const name, const int optional) {
    char *cfgFile;
...
int cnfparamGetIdx(struct cnfparamblk *params, const char *name) {
    int i;
    for (i = 0; i < params->nParams; ++i)
        if (!strcmp(params->descr[i].name, name)) break;
    if (i == params->nParams) i = -1; /* not found */
    return i;
}
...
static inline int isodigit(uchar c) {
    return (c >= '0' && c <= '7');
}
...
const char *tokenval2str(const int tok) {
    if (tok < 256) return "";
    switch (tok) {
        case NAME:
            return "NAME";
        ...
        case BEGIN_ACTION:
            return "BEGIN_ACTION";
        case BEGIN_PROPERTY:
            return "BEGIN_PROPERTY";
        case BEGIN_CONSTANT:
            return "BEGIN_CONSTANT";
        case BEGIN_TPL:
            return "BEGIN_TPL";
        case BEGIN_INCLUDE:
            return "BEGIN_INCLUDE";
        case BEGIN_RULESET:
            return "BEGIN_RULESET";
        case STOP:
            return "STOP";
        case SET:
            return "SET";
        ...
        case PROPFILT:
            return "PROPFILT";
        case BSD_TAG_SELECTOR:
            return "BSD_TAG_SELECTOR";
        case BSD_HOST_SELECTOR:
            return "BSD_HOST_SELECTOR";
        case IF:
            return "IF";
        case THEN:
            return "THEN";
